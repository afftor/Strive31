...
func show_(dict):
	...
	get_node("textpanel/RichTextLabel").set_size(Vector2(get_node("textpanel/RichTextLabel").get_size().x, get_node("textpanel/RichTextLabel").get_v_scroll().get_max()))
...
func advance():
	if stage == 'epilogue':
		...
		if patrons.size() == 0:
			...
			get_node("TextureFrame/RichTextLabel1").visible = true
			get_node("TextureFrame/VBoxContainer").visible = true
			patronlist()
	elif stage == 'alise' && get_node("TextureFrame/alise").visible == false:
		get_node("TextureFrame").visible = true
		get_node("TextureFrame/RichTextLabel").visible = false
