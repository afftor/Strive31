...
basemana = 3,
alwaysallowed = false,
slavereqs = "person.arms in ['normal','fur_covered']",
...
basemana = 4,
alwaysallowed = false,
slavereqs = 'globals.sizearray.find(person.titssize) >= 2',
playerreqs = 'globals.partner.penis != "none"',
receive = false,
lusteffect = 30,
},
...
basemana = 4,
alwaysallowed = false,
slavereqs = 'globals.hairlengtharray.find(person.hairlength) >= 1',
playerreqs = 'globals.partner.penis != "none"',
receive = false,
lusteffect = 15,
},
...
basemana = 5,
alwaysallowed = false,
slavereqs = 'globals.longtails.find(person.tail) >= 0',
playerreqs = 'globals.partner.penis != "none"',
receive = false,
lusteffect = 20,
},
...
basemana = 4,
alwaysallowed = false,
slavereqs = 'person.sexuals.unlocks.has("anal")',
playerreqs = 'true',
receive = false,
lusteffect = 30,
},
...
basemana = 5,
alwaysallowed = false,
slavereqs = 'person.penis != "none" || person.sexuals.unlocks.has("toys") == true',
playerreqs = 'globals.partner.vagina != "none"',
receive = true,
lusteffect = 35,
},
...
basemana = 5,
alwaysallowed = false,
slavereqs = "person.mods.has('hollownipples') == true",
playerreqs = 'globals.partner.penis != "none"',
receive = false,
lusteffect = 35,
},
...
basemana = 3,
alwaysallowed = false,
slavereqs = 'person.lactation == true',
playerreqs = 'true',
receive = false,
lusteffect = 20,
},
...
func pussysex(person):
	...
	text += "You don't keep $him waiting for very long. Leaning down, you press your mouth against $his " + globals.fastif(person.bodyshape == 'bestial', "muzzle", "own") + " eagerly tasting inside, even as $he starts to respond.  Your hands stroke over $his body, things growing more heated as you line up the tip of " +  globals.player.dictionaryplayer("$penis") + " sliding up and down $his lower lips. $He's already more than soaked, and you have no trouble at all parting $his folds and pushing into $his body. $He " + globals.fastif(person.vagvirgin == true, "winces rather hard, the resistance within $him breaking away a moment later as $he gasps, then", "") + globals.fastif(person.sexuals.actions.pussy >= 4, " gives a lusty moan and", "") + " squirms under you as you start a steady pace of thrusts into $his body, holding $him to you as $his legs wrap around your back almost instinctively" + globals.fastif(person.tail != "none", " and $his tail wraps around your leg", "" )+ ". $His insides are hot and wet against your " +  globals.player.dictionaryplayer("$penis") + ", feeling amazing as you slide in and out of $his body. " + globals.fastif(person.vagvirgin == true, "A small amount of pink tinting to the fluids around your connection prove it.  You've taken $his virginity. ", "")
	text += "\n\nIt doesn't take long before you feel yourself nearing the end, and pick up the pace pounding into $his body, possessively kissing $him as you hold $him down. Taken by $his own instincts, $name moans into the kiss as $his whole body rocks under you. " +  globals.fastif(globals.player.penis != "none", "$He clenches down a moment later, shivering as you feel $his walls start to squeeze and contract around you, almost begging you for your seed. " , "" ) + globals.fastif(person.penis != "none", "$His own dick twitches as well, messily splattering up $his stomach.", "") + globals.fastif(globals.player.penis != "none", " Soon you slam in as deep as you can go and flooding $his core with your cum. ", "")
	return text

func pussyrape(person):
	var text = ''
	if person.traits.find('Likes it rough') >= 0:
		text += "$name makes a show of struggling "
	else:
		text += "$name struggles "
	text += "as you forcefully toss $him onto the bed. "
	if person.bodyshape in ['humanoid', 'bestial']:
		if person.tail != "none":
			text += "Forcing $his legs apart you roughly push $his " + person.tail + " tail out of the way exposing $his pussy to you. "
		else:
			text += "Forcing $his legs apart you open $his pussy to you. "
	else:
		text += "Grabbing $him by $his " + person.legs + "legs you reveal $his pussy. "
	text += "Pressing your " + globals.player.dictionaryplayer("$penis") + " against $his groin $he begs you not to do it. With a single thrust you slam your " +  globals.player.dictionaryplayer("$penis") + " all the way into $his slit. $He " + globals.fastif(person.vagvirgin == true, "screams in pain as your " + globals.player.dictionaryplayer("$penis") + " tears through $his virginity.", "yells out in protest at your sudden assault.") + "\n\n Wraping your arms about $him you hold $him in position as you thrust your " +  globals.player.dictionaryplayer("$penis") + " in and out of $his pussy with hard violent strokes. $He squirms trying to break free and get away but you hold on tightly. Tears start falling down $his cheeks and $he begs you to stop. $His desperation brings you closer to climax."
	return text

func analsex(person):
	var text = "With a simple gesture, you order $name to strip, moving around behind $him as $he does. " 
	if person.sexuals.actions.ass >= 5:
	...
	if person.tail != 'none' && person.sexuals.actions.size() >= 6: 
		text +="smirking as $he eagerly lifts $his tail out of the way for you, then "
	elif person.tail != 'none':
		text += "lifting $his tail out of the way and "
	text += globals.player.dictionaryplayer("pressing your $penis") + " up and nestling it between the cheeks of $his ass.  Your hands squeeze both sides of $his hips as you pull $him back against you and grind for a moment, drawing out " + globals.fastif(person.sexuals.actions.ass >= 4, "an excited groan ", "a nervous whine ") + "from $him.\n\n"
	...
	if person.sexuals.actions.size() >= 8 && person.vagina != "none": 
	...
	elif person.skincov == 'scales':
		text += "slicking $his scales with $his fluids"
	elif person.skincov == 'feathers':
		text += "saturating the feathers with $his fluids"
	text += ". You unceremoniously pull out of $him, stepping back to admire your handiwork.  Your seed slides slowly down between $his ass cheeks and $his thighs, and $he gives a " + globals.fastif(person.sexuals.actions.size() >= 3, "pleased purr, looking back over $his shoulder at you as $he licks $his fingers off of $his own juices. "," contented sigh, sagging a little after you let $him go. ") + "You leave $him to clean up as you continue about your day."
	return text
	
func analsexrapelike(person):
	var text = "With a simple gesture, you order $name to strip, moving around behind $him as $he does. " 
	if person.sexuals.actions.ass >= 5:
...
func blowjobsex(person):
	var text = ''
	text += "You ask $name to give you a blowjob. After a moment of consideration, $he accepts.\n\nYou free your penis from your pants, as $he shimmies down your body $his face drawing level with your groin. Eagerly, $he takes your member into $his "+ globals.fastif(person.bodyshape == 'bestial', "muzzle", "mouth") + " and begins to carefully suck on you while maintaining eye contact to measure your reaction.\n\n"
	if rand_range(0,1) >= 0.5:
		text += "As you get closer to climax, you order $him to swallow everything. Your semen gushes into $his mouth" +globals.fastif(person.sexuals.actions.blowjob < 3, ", nearly making $him choke", "") + ".  $name shows you the creamy covering of $his tongue proudly before swallowing it down with a grin. After thanking $him, you leave $him to $his duties."
	else:
		text += "As you climax, you pull out of $his mouth and covering $his "+ globals.fastif(person.beauty >= 70, "pretty", "") + " face and "+person.haircolor + " hair with your seed."
	return text
	
func blowjobrape(person):
	var text = ''
...
func getscene(sexaction, person, consensual, hole):
	...
	for i in dict.values():
		...
		if i.has('consensual'):
			if i.consensual != consensual:
				check = false
		if i.has('slavesex'):
			if person.sex != i.slavesex:
				check = false
		if i.has('notslavesex'):
			if person.sex == i.notslavesex:
				check = false
		if i.has('titsize'):
			if globals.sizearray.find(person.titssize) > globals.sizearray.find(i.titsize):
				check = false
		if i.has('nottitsize'):
			if globals.sizearray.find(person.titssize) <= globals.sizearray.find(i.nottitsize):
				check = false
		if i.has('loyaltyabove'):
			if person.loyal < i.loyaltyabove:
				check = false
		if i.has('loyaltybelow'):
			if person.loyal >= i.loyaltybelow:
				check = false
		if i.has('pchaspenis'):
